...
void Voronoi::InsertParabola(VPoint *p)
{
  ...
  if (root->isLeaf && (root->site->y - p->y < 1)) // degenerovaný pøípad - obì
                                                  // spodní místa ve stejné
                                                  // výšce
  {
    VPoint *fp = root->site;
    root->isLeaf = false;
    ...
    VPoint *s = new VPoint((p->x + fp->x) / 2, height);  // zaèátek hrany
                                                         // uprostøed míst
    points.push_back(s);

    if (p->x > fp->x) root->edge = new VEdge(s, fp, p);  // rozhodnu, který
                                                         // vlevo, který vpravo
